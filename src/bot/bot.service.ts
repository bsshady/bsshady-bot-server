...
export class BotService implements OnModuleInit {
  ...
  async botMessage() {
    ...
    bot.on('new_chat_members', (msg) =>
      bot.sendMessage(
        msg.chat.id,
        `Привет, ${msg.new_chat_members[0].first_name}! Добро пожаловать наша госпожа!!! Я бот созданный твоим мужем! @bshadle тебя очень любит!! мяу`,
      ),
    );
    ...
    bot.on('message', async (msg) => {
      if (msg?.reply_to_message) {
        ...
        if (
          msg.reply_to_message.from.username === 'bsshady_bot' ||
            ...
            [
              {
                ...
                url: 'https://bsshady-bot-client.vercel.app/',
              },
            ],
          ],
        },
